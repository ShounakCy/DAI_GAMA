...
species Queen skills:[fipa] {
   ...
   reflex reactToMessages when: !empty(informs) {
    	...
    	write "Current Row initial = "+ currentRow;
    	if(list(msg.contents)[0] = 'FindYourPosition') {
    		tryToFindPosition <- true;
    		write name + ": I'm looking for a new position";
    	} else if (list(msg.contents)[0] = 'RePosition') {
    		write "current Row now = "+ currentRow;
    		currentRow <- (currentRow + 1) mod N;
    		write "current Row then = "+ currentRow;
    		foundMyPosition <- false;
